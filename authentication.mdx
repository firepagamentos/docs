...
```bash cURL - PIX (Conforme API Gateway v11)
curl --request GET \
...
<CardGroup cols={3}>
  <Card title="PIX" icon="qrcode">
    **pixWebhookUrl**
    Cash In e Cash Out PIX
  </Card>
  <Card title="Boleto" icon="file-invoice">
    **bankSlipWebhookUrl**
    Confirmações e vencimentos
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="Gestão de Chaves">
    - **Revogação imediata** possível via dashboard
    - **Geração de nova chave** revoga a anterior (após transição)
    - **Rotação periódica** recomendada para segurança
    - **Backup seguro** antes de revogar chave atual
  </Accordion>
