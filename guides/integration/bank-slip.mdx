...
curl -X GET "https://api-gateway.firebanking.dev/bank-slip/v1/boleto_abc123def456/pdf" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  --output boleto.pdf
```

```javascript JavaScript - PDF Base64
const response = await fetch('https://api-gateway.firebanking.dev/bank-slip/v1/boleto_abc123def456/pdf?format=base64', {
  headers: {
    'x-api-key': 'fbk_test_SUA_CHAVE_AQUI'
  }
});
...
curl -X GET "https://api-gateway.firebanking.dev/bank-slip/v1/boleto_abc123def456" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI"
```

```bash cURL - Consulta por External ID
curl -X GET "https://api-gateway.firebanking.dev/bank-slip/v1/payment?externalId=boleto-001" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI"
```
</CodeGroup>
...
curl -X DELETE "https://api-gateway.firebanking.dev/bank-slip/v1/boleto_abc123def456" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  -H "Content-Type: application/json" \\
  -d '{
