...
<CardGroup cols={2}>
  <Card title="Sandbox Boleto" icon="flask">
    **Base URL**: `https://api-gateway.firebanking.dev/v1/bank-slip/v1`

    Use para testes e desenvolvimento
  </Card>
  <Card title="Produção Boleto" icon="globe">
    **Base URL**: `https://api-gateway.firebanking.com.br/v1/bank-slip/v1`
...
curl -X POST "https://api-gateway.firebanking.dev/v1/bank-slip/v1/payment" \\
...
const boletoData = {
  ...
  externalId: "boleto-001",
  dueDate: "2024-02-15",
  fineRate: 2.0, // 2% de multa após vencimento
  interestRate: 1.0, // 1% ao mês de juros
  instructions: [
    "Pagamento via boleto bancário",
    "Não receber após o vencimento"
  ],
...
const response = await fetch('https://api-gateway.firebanking.dev/v1/bank-slip/v1/payment', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api-gateway.firebanking.dev/v1/bank-slip/v1/payment',
    json=boleto_data,
    headers={
        'x-api-key': 'fbk_test_SUA_CHAVE_AQUI',
        'Content-Type': 'application/json'
...
<CodeGroup>
```bash cURL - Baixar PDF
curl -X GET "https://api-gateway.firebanking.dev/v1/bank-slip/v1/boleto_abc123def456/pdf" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  --output boleto.pdf
```

```javascript JavaScript - PDF Base64
const response = await fetch('https://api-gateway.firebanking.dev/v1/bank-slip/v1/boleto_abc123def456/pdf?format=base64', {
  headers: {
    'x-api-key': 'fbk_test_SUA_CHAVE_AQUI'
  }
});
...
console.log('PDF Base64:', pdfData.base64);

// Para baixar no navegador
...
curl -X GET "https://api-gateway.firebanking.dev/v1/bank-slip/v1/boleto_abc123def456" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI"
```

```bash cURL - Consulta por External ID
curl -X GET "https://api-gateway.firebanking.dev/v1/bank-slip/v1/payment?externalId=boleto-001" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI"
```
</CodeGroup>
...
curl -X DELETE "https://api-gateway.firebanking.dev/v1/bank-slip/v1/boleto_abc123def456" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  -H "Content-Type: application/json" \\
  -d '{
...
curl -X PATCH "https://api-gateway.firebanking.dev/v1/bank-slip/v1/boleto_abc123def456" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  -H "Content-Type: application/json" \\
  -d '{
...
curl -X POST "https://api-gateway.firebanking.dev/v1/bank-slip/v1/boleto_abc123def456/simulate-payment" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  -H "Content-Type: application/json" \\
  -d '{
    "paidAmount": 145.75,
