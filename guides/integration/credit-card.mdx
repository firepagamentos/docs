...
<CodeGroup>
```bash cURL
curl -X POST "https://api-gateway.firebanking.dev/v1/credit-card/v1/payment" \\
...
const response = await fetch('https://api-gateway.firebanking.dev/v1/credit-card/v1/payment', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api-gateway.firebanking.dev/v1/credit-card/v1/payment',
    json=payment_data,
    headers={
        'x-api-key': 'fbk_test_SUA_CHAVE_AQUI',
        'Content-Type': 'application/json'
...
curl_setopt_array($curl, [
    CURLOPT_URL => 'https://api-gateway.firebanking.dev/v1/credit-card/v1/payment',
...
```json 3x sem juros
{
  "purchaseValue": 300,
  "installments": 3,
  "installmentType": "MERCHANT_INTEREST_FREE"
}
```

```json 6x com juros (cliente)
{
  "purchaseValue": 600,
  "installments": 6,
  "installmentType": "CUSTOMER_INTEREST",
  "interestRate": 2.5
}
```
</CodeGroup>
...
| Tipo | Descrição | Quem Paga Juros |
|------|-----------|-----------------|
| `MERCHANT_INTEREST_FREE` | Sem juros (lojista absorve) | Lojista |
| `CUSTOMER_INTEREST` | Com juros para o cliente | Cliente |
...
curl -X POST "https://api-gateway.firebanking.dev/v1/credit-card/v1/token" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  -H "Content-Type: application/json" \\
  -d '{
    "card": {
...
curl -X GET "https://api-gateway.firebanking.dev/v1/credit-card/v1/payment/{transactionId}" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI"
```

```bash cURL - Consultar por External ID
curl -X GET "https://api-gateway.firebanking.dev/v1/credit-card/v1/payment?externalId=card-payment-001" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI"
```
</CodeGroup>
...
curl -X POST "https://api-gateway.firebanking.dev/v1/credit-card/v1/payment/{transactionId}/capture" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  -H "Content-Type: application/json" \\
  -d '{
    "amount": 100
...
curl -X POST "https://api-gateway.firebanking.dev/v1/credit-card/v1/subscription" \\
  -H "x-api-key: fbk_test_SUA_CHAVE_AQUI" \\
  -H "Content-Type: application/json" \\
  -d '{
    "amount": 29.90,
...
<CardGroup cols={2}>
  <Card title="Webhooks Avançados" icon="webhook" href="/api-reference/credit-card/webhooks">
    Configure notificações para todos os eventos
  </Card>
  <Card title="Referência da API" icon="code" href="/api-reference/credit-card/payment/create">
