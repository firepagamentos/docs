...
api: 'POST https://api.firebanking.com/v2/payment'
...
<CodeGroup>
```bash cURL
curl -X POST "https://api.firebanking.com/v2/payment" \
...
const pixPayment = await fetch('https://api.firebanking.com/v2/payment', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.firebanking.com/v2/payment',
    json=pix_data,
    headers={
        'Authorization': 'Bearer fbk_live_sua_chave_aqui',
        'Content-Type': 'application/json'
...
curl_setopt_array($curl, [
    CURLOPT_URL => 'https://api.firebanking.com/v2/payment',
...
```json 200 OK
{
  ...
  "description": "Compra na loja virtual",
  "qrCode": {
    "text": "00020101021226830014br.gov.bcb.pix2561api.firebanking.com/pix/v2/12345678901234567890520400005303986540515.005802BR5925Loja Virtual FireBanking6009SAO PAULO62070503***630445E3",
...
```json 400 Bad Request
{
  "error": {
    ...
    "details": "Valor recebido: 0 centavos",
...
function showPixPayment(pixData) {
  ...
  document.getElementById('amount').textContent = (pixData.amount / 100).toLocaleString('pt-BR', {
...
async function checkPaymentStatus(pixId) {
  try {
    const response = await fetch(`https://api.firebanking.com/v2/payment/${pixId}`, {
      headers: {
        'Authorization': 'Bearer fbk_live_sua_chave_aqui'
      }
    });
