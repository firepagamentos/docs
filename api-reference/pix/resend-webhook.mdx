...
<Warning>
**Rate Limit**: Este endpoint possui rate limiting de **2 requisições por minuto** para prevenir sobrecarga.
</Warning>
...
<ResponseField name="429" type="error">
  Rate limit excedido (máximo 2 requisições por minuto)
</ResponseField>
...
<AccordionGroup>
  ...
  <Accordion title="Rate limit excedido">
    ...
    ```
    Você excedeu o limite de 2 requisições por minuto. Aguarde antes de tentar novamente.
  </Accordion>
</AccordionGroup>
...
<Warning>
**Limite**: 2 requisições por minuto por business

**TTL**: 60 segundos
</Warning>

Se você precisar reenviar múltiplos webhooks:
1. Envie as requisições com intervalo de 30+ segundos entre elas
2. Considere usar ferramentas de retry com exponential backoff
3. Para volumes maiores, entre em contato com o suporte
...
<CardGroup cols={2}>
  ...
  <Card title="Respeite Rate Limit" icon="clock">
    Aguarde 30s entre requisições para evitar 429
  </Card>
</CardGroup>
