---
title: 'Criar Boleto'
api: 'POST https://api.firebanking.com/v1/payment'
...
<ParamField body="amount" type="integer" required>
  Valor do boleto em centavos (R$ 100,00 = 10000). Mínimo: 250 (R$ 2,50)
</ParamField>
...
<ParamField body="fine_amount" type="integer">
  Multa em centavos para pagamento após vencimento. Padrão: 2% do valor
</ParamField>

<ParamField body="interest_amount" type="integer">
  Juros diários em centavos após vencimento. Padrão: 0,033% ao dia
</ParamField>
...
curl -X POST "https://api.firebanking.com/v1/payment" \
  -H "Authorization: Bearer fbk_live_sua_chave_aqui" \
...
const boletoData = {
  ...
  fine_amount: 500, // R$ 5,00 de multa
  interest_amount: 83, // R$ 0,83 de juros por dia
  callback_url: 'https://meusite.com/webhooks/boleto',
  instructions: [
    'Pagamento referente à mensalidade de Janeiro/2024',
    'Após vencimento será cobrada multa de 2%',
...
const response = await fetch('https://api.firebanking.com/v1/payment', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer fbk_live_sua_chave_aqui',
...
response = requests.post(
    'https://api.firebanking.com/v1/payment',
    json=boleto_data,
    headers={
        'Authorization': 'Bearer fbk_live_sua_chave_aqui',
        'Content-Type': 'application/json'
...
curl_setopt_array($curl, [
    CURLOPT_URL => 'https://api.firebanking.com/v1/payment',
...
```json 201 Created
{
  ...
  "pdf_url": "https://api.firebanking.com/v1/boleto/bol_1234567890123456/pdf",
...
function displayBoleto(boletoData) {
  ...
  document.getElementById('boleto-amount').textContent = 
    (boletoData.amount / 100).toLocaleString('pt-BR', {
      style: 'currency',
      currency: 'BRL'
    });
...
function sendByEmail() {
  ...
  const body = `Olá!\n\nSeu boleto está pronto para pagamento:\n\n` +
    `Valor: R$ ${(currentBoleto.amount / 100).toFixed(2)}\n` +
...
async function checkBoletoStatus(boletoId) {
  try {
    const response = await fetch(`https://api.firebanking.com/v1/payment/${boletoId}`, {
      headers: {
        'Authorization': 'Bearer fbk_live_sua_chave_aqui'
      }
    });
...
function showPaymentConfirmed(boleto) {
  ...
  notification.innerHTML = `
    ...
    <p>Valor pago: ${(boleto.paid_amount / 100).toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}</p>
  `;
