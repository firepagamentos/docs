...
## Bem-vindos às APIs Comuns FireBanking
...
<CardGroup cols={2}>
  ...
  <Card title="Monitoramento" icon="activity" href="/api-reference/health/check">
    Verifique o status e saúde da API
  </Card>
...
curl -X GET "https://api-gateway.firebanking.dev/health/check" \
...
| `/health/check` | 300 req/min | Por IP |
...
<CardGroup cols={2}>
  ...
  <Card title="Monitorar Sistema" icon="activity" href="/api-reference/health/check">
    Implemente health checks na sua infraestrutura
  </Card>
...
<Note>
  **Dica**: Comece sempre verificando o status da API com `/health/check` antes de implementar outros endpoints.
</Note>