...
description: 'Receba notificações automáticas quando um boleto for registrado com sucesso no banco'
---
...
- Código de barras e linha digitável gerados
- PDF do boleto disponível para download
...
async function processBoletoConfirmed(payload) {
  ...
  try {
    ...
    console.error(`Erro ao processar confirmação do boleto ${boleto_id}:`, error);
    throw error;
  }
}
```
