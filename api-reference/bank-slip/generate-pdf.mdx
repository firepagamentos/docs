...
def generate_boleto_pdf(boleto_id):
    """Gera PDF do boleto e retorna URL de acesso"""
...
def download_boleto_pdf(boleto_id, filename='boleto.pdf'):
    ...
    # Gerar URL do PDF
    result = generate_boleto_pdf(boleto_id)
    pdf_url = result['pdfAccessUrl']
...
print(f'PDF disponível em: {pdf_data["pdfAccessUrl"]}')
```

```php PHP
<?php
function generateBoletoPDF($boletoId) {
    $curl = curl_init();

    curl_setopt_array($curl, [
        CURLOPT_URL => "https://api-gateway.firebanking.com.br/bank-slip/v1/payment/{$boletoId}/generate-pdf",
...
function downloadBoletoPDF($boletoId, $filename = 'boleto.pdf') {
    // Gerar URL do PDF
    $result = generateBoletoPDF($boletoId);
    $pdfUrl = $result['pdfAccessUrl'];
...
echo "PDF disponível em: " . $pdfData['pdfAccessUrl'];
?>
```
</CodeGroup>
