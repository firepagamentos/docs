...
description: 'Gere e baixe o arquivo PDF de um boleto bancário para impressão ou compartilhamento'
---
...
<ParamField query="format" type="string">
  Formato do retorno: `url` (padrão) ou `base64`
</ParamField>
...
curl -X POST "https://api-gateway.firebanking.com.br/v1/payment/bol_1234567890123456/generate-pdf" \
  -H "x-api-key: fbk_live_sua_chave_aqui"
```

```bash cURL - Por External ID
curl -X POST "https://api-gateway.firebanking.com.br/v1/payment/pedido-123/generate-pdf?type=external" \
  -H "x-api-key: fbk_live_sua_chave_aqui"
```

```bash cURL - Retornar Base64
curl -X POST "https://api-gateway.firebanking.com.br/v1/payment/bol_1234567890123456/generate-pdf?format=base64" \
  -H "x-api-key: fbk_live_sua_chave_aqui"
```
...
async function generateBoletoPDF(boletoId) {
  const response = await fetch(`https://api-gateway.firebanking.com.br/v1/payment/${boletoId}/generate-pdf`, {
    method: 'POST',
    headers: {
...
def generate_boleto_pdf(boleto_id, format_type='url'):
    ...
    response = requests.post(
        f'https://api-gateway.firebanking.com.br/v1/payment/{boleto_id}/generate-pdf',
...
print(f'PDF disponível em: {pdf_data["pdf_url"]}')
```
...
function generateBoletoPDF($boletoId, $format = 'url') {
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL => "https://api-gateway.firebanking.com.br/v1/payment/{$boletoId}/generate-pdf?format={$format}",
...
echo "PDF disponível em: " . $pdfData['pdf_url'];
?>
```
</CodeGroup>
...
```json 422 Unprocessable Entity
{
  "error": {
    ...
    "message": "PDF não pode ser gerado",
...
app.post('/api/boleto/:id/pdf', async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://api-gateway.firebanking.com.br/v1/payment/${id}/generate-pdf?format=${format}`,
      {
        method: 'POST',
        headers: {
...
<CardGroup cols={2}>
  ...
  <Card title="Relatórios" href="/api-reference/reports/bank-slips">
    Relatórios de boletos emitidos
  </Card>
</CardGroup>