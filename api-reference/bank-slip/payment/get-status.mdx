...
Consulta o status atualizado de um pagamento de boleto diretamente na Celcoin e sincroniza automaticamente com o banco de dados local. Útil para:
...
<Info>
Este endpoint consulta o status **em tempo real** na Celcoin e atualiza automaticamente o banco de dados local se houver divergência.
</Info>
...
2. Consulta o status na Celcoin em tempo real
...
<Warning>
**Fail-safe**: Se a consulta na Celcoin falhar, o endpoint retorna o status local sem erro, garantindo que você sempre tenha uma resposta.
</Warning>
...
// O status foi automaticamente sincronizado com a Celcoin
console.log('Status atualizado:', statusResponse.status);
```

## Mapeamento de Status

| Status Celcoin | Status Interno | Descrição |
|----------------|----------------|-----------|
