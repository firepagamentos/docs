...
<ParamField body="reason" type="string">
  Motivo do cancelamento (máximo 200 caracteres)
</ParamField>

<ParamField body="metadata" type="object">
  Dados adicionais sobre o cancelamento
</ParamField>

## Exemplo de Requisição

<CodeGroup>
```bash cURL - Cancelamento Simples
curl -X PATCH "https://api-gateway.firebanking.com.br/v1/payment/bol_1234567890123456/cancel" \
...
curl -X PATCH "https://api-gateway.firebanking.com.br/v1/payment/pedido-123/cancel?type=external" \
  -H "x-api-key: fbk_live_sua_chave_aqui" \
  -H "Content-Type: application/json" \
  -d '{
    ...
    "metadata": {
      "cancelled_by": "system",
      "original_status": "awaiting_stock"
    }
  }'
```
...
async function cancelBoleto(boletoId, reason) {
  const response = await fetch(`https://api-gateway.firebanking.com.br/v1/payment/${boletoId}/cancel`, {
    method: 'PATCH',
    headers: {
...
def cancel_boleto(boleto_id, reason, metadata=None):
    ...
    response = requests.patch(
        f'https://api-gateway.firebanking.com.br/v1/payment/{boleto_id}/cancel',
        json=data,
...
def cancel_boleto_by_order(order_id, reason):
    ...
    response = requests.patch(
        f'https://api-gateway.firebanking.com.br/v1/payment/{order_id}/cancel',
...
function cancelBoleto($boletoId, $reason, $metadata = []) {
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL => "https://api-gateway.firebanking.com.br/v1/payment/{$boletoId}/cancel",
...
<ResponseExample>
```json 200 OK - Cancelamento Realizado
{
...
```json 400 Bad Request - Boleto Não Pode Ser Cancelado
{
  "error": {
    "code": "BOLETO_CANNOT_BE_CANCELLED",
    "message": "Boleto não pode ser cancelado",
    "details": "Apenas boletos com status 'registered' podem ser cancelados. Status atual: paid",
...
app.patch('/api/boleto/:id/cancel', async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://api-gateway.firebanking.com.br/v1/payment/${id}/cancel`,
      {
        method: 'PATCH',
        headers: {
