...
app.post('/webhooks/credit-card', (req, res) => {
    ...
    switch (event.event) {
        ...
        case 'subscription.payment_successful':
            console.log('Cobrança recorrente bem-sucedida:', event.data.subscription_id);
...
### Dashboard
Monitore seus webhooks em tempo real através do [Dashboard FireBanking](https://dashboard.firebanking.com):
...
<Info>
  **Dica**: Use ferramentas como [webhook.site](https://webhook.site) para testar e depurar seus webhooks durante o desenvolvimento.
</Info>