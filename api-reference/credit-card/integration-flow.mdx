---
title: 'Fluxo de Integração - Cartão de Crédito'
description: 'Guia completo do fluxo de integração para processar pagamentos com cartão de crédito'
---
...
<Steps>
  ...
  <Step title="Processamento pela Operadora">
    Validações de segurança, limite e autorização da transação
  </Step>
  <Step title="Confirmação Final">
...
const processPayment = async (formData) => {
  try {
    ...
    const response = await fetch('https://api-gateway.firebanking.com.br/credit-card/v1/payment', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        purchaseValue: formData.amount,
        installments: formData.installments,
        externalId: generateOrderId(),
        purchaseTitle: formData.description,
        ...
        buyer: {
          ...
          address: {
            ...
            district: formData.district || "Centro",
            street: formData.street || "Rua Principal",
            zipCode: formData.zipCode || "01000000",
            number: formData.number || "123"
          }
        }
      })
    });
    ...
    handlePaymentError({ message: 'Erro de conexão. Tente novamente.' });
  } finally {
    showLoading(false);
  }
};
    ...
    response = requests.post(
        ...
        json={
            ...
            "buyer": {
                ...
                "phone": payment_data.get('phone', '11999999999'),
                "countryCode": "+55",
                "address": {
                    "country": "BR",
                    "state": payment_data.get('state', 'SP'),
                    "city": payment_data.get('city', 'São Paulo'),
                    "district": payment_data.get('district', 'Centro'),
                    "street": payment_data.get('street', 'Rua Principal'),
                    "zipCode": payment_data.get('zip_code', '01000000'),
                    "number": payment_data.get('number', '123')
                }
            }
        }
    )
...
A API retorna uma resposta inicial com o status da transação:
...
Com base na análise, você receberá um webhook com o resultado final:

**Transação Aprovada** - `PAID`:
```json
{
  "product": "CREDIT_CARD",
...
|--------|-----------|---------------|
| `PROCESSING` | Transação em análise | Aguardar webhook final |
| `PAID` | Pagamento aprovado | Liberar produto/serviço |
| `DENIED` | Pagamento negado | Informar cliente e oferecer alternativa |
| `CANCELED` | Transação cancelada | Informar cancelamento |
...
<CardGroup cols={2}>
  <Card title="API Reference - Criar Pagamento" href="/api-reference/credit-card/create">
    Documentação completa da API de cartão
  </Card>
  <Card title="Webhooks - Cartão de Crédito" href="/api-reference/credit-card/webhooks">
    Implementar recebimento de confirmações
  </Card>
  <Card title="Estornos - Cartão de Crédito" href="/api-reference/credit-card/refund">
    Processar devoluções de pagamento
  </Card>
  <Card title="Sandbox - Ambiente de Testes" href="/testing/sandbox">
    Teste sua integração com cartões fictícios
  </Card>
</CardGroup>